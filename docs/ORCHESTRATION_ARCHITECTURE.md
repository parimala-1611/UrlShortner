...
metrics. See [`SCENARIOS.md`](./SCENARIOS.md) for the three scenarios it was used to
drive, and [`SETUP.md`](./SETUP.md) to run it yourself.
...
the `test` stage's exit gate is "the actual `mvnw test` process exited 0"; the `docs`
stage's is "the actual `git diff` includes the expected doc files"; `compliance`'s is
"no guardrail check failed."
